	...
	static upgradeArgs({args, kwArgs}: {args: Args, kwArgs: KWArgs}): {args: Args, kwArgs: KWArgs} {
		...
		case '-activate': {
			...
			if ([
				...
				'electricterrain', 'mistyterrain', 'psychicterrain', 'evanescediffusion', 'allurediffusion', 'telepathy', 'stickyhold', 'suctioncups', 'aromaveil',
				'flowerveil', 'sweetveil', 'disguise', 'mimictree', 'safetygoggles', 'protectivepads', 'berserkterrain', 'sporeshield', 'fathersbloodlust', 'mothersblessing',
			].includes(id)) {
				if (target) {
					kwArgs.of = pokemon;
			...
			if ([
				'shadowhold', 'silksnare', 'spiderweb', 'sporeslash', 'bind', 'wrap', 'clamp', 'whirlpool', 'firespin', 'bubbleprison', 'magmastorm', 'sandtomb', 'infestation', 'snaptrap', 'thundercage', 'trapped',
			].includes(id)) {
			...
			if (id === 'symbiosis' || id === 'poltergeist') {
				...
				kwArgs.number = arg3;
			} else if (id === 'skillswap' || id === 'mummy' || id === 'wanderingspirit' || id === 'hivemind') {
				kwArgs.ability = arg3;
	...
	parseArgsInner(args: Args, kwArgs: KWArgs) {
		...
		case '-fail': {
			...
			if (['desolateland', 'primordialsea'].includes(blocker) &&
				!['sunnyday', 'raindance', 'sandstorm', 'hail', 'miasma', 'shadowsky'].includes(id)) {
