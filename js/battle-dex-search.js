...
if(format==='anythinggoes'||format==='hypnomons'||format.endsWith('ag')||format.startsWith('ag')){
tierSet=tierSet.slice(slices.AG);
}else if(format.includes('hackmons')||format.endsWith('bh'))tierSet=tierSet.slice(slices.AG);else
...
var species=this.dex.species.get(row[1]);for(var _i5=0;_i5<
filters.length;_i5++){var _ref7=filters[_i5];var filterType=_ref7[0];var value=_ref7[1];
switch(filterType){
case'type':
...
var ability=this.dex.abilities.get(row[1]);for(var _i7=0;_i7<
filters.length;_i7++){var _ref14=filters[_i7];var filterType=_ref14[0];var value=_ref14[1];
switch(filterType){
case'pokemon':
...
results.length;_i8++){var _this$dex$items$get$i;var row=results[_i8];
...
if(!filters)return true;
if(row[0]!=='ability')return true;
var ability=this.dex.abilities.get(row[1]);for(var _i9=0;_i9<
filters.length;_i9++){var _ref15=filters[_i9];var filterType=_ref15[0];var value=_ref15[1];
switch(filterType){
case'pokemon':
if(!Dex.hasAbility(this.dex.species.get(value),ability.name))return false;
break;}
...
case'naturalgift':
return species.baseSpecies==='Kiwuit';
case'naturepower':
...
return['skilllink','technician'].includes(abilityid);
case'synchronoise':
return abilityid==='melodyallegretto';
case'technoblast':
...
if(
(_this$formatType3=this.formatType)!=null&&_this$formatType3.startsWith('dlc1')&&(_BattleTeambuilderTab=
...
var types=[];
var baseSpecies=dex.species.get(species.changesFrom||species.name);
if(!species.battleOnly)types.push.apply(types,species.types);
var prevo=species.prevo;
while(prevo){
var prevoSpecies=dex.species.get(prevo);
...
var move=this.dex.moves.get(row[1]);for(var _i13=0;_i13<
filters.length;_i13++){var _ref16=filters[_i13];var filterType=_ref16[0];var value=_ref16[1];
switch(filterType){
case'type':
...
shadowanvil:120,shadowtrip:120,shadowcentrifuge:150,shadowdart:150,shadowhalf:1350};
...
throw new Error("invalid sortcol");
};return BattleMoveSearch;}(BattleTypedSearch);BattleMoveSearch.GOOD_STATUS_MOVES=['aggregate','agility','aromatherapy','auroraveil','autotomize','badseed','banefulbunker','batonpass','bellydrum','bulkup','cagematch','calmmind','chillingrime','clangoroussoul','coil','cottonguard','courtchange','curse','darkvoid','defog','destinybond','detect','disable','dragondance','drainingkiss','drypass','encore','extremeevoboost','geomancy','glare','haze','healbell','healingwish','healorder','heartswap','honeclaws','kappo','kingsshield','irondefense','leechseed','lightscreen','lovelykiss','lunardance','magiccoat','maxguard','memento','milkdrink','moonlight','morningsun','nastyplot','naturalshift','naturesmadness','noretreat','obstruct','orbitalshift','painsplit','parasiticdrain','partingshot','perishsong','primalinstinct','protect','quiverdance','recover','reflect','reflecttype','regroup','rest','roar','rockpolish','roost','shellsmash','shiftgear','shoreup','slackoff','sleeppowder','sleeptalk','softboiled','spikes','spikyshield','spore','stealthrock','stickyweb','strengthsap','stripestyle','substitute','switcheroo','swordsdance','synthesis','tailglow','tailwind','taunt','thunderwave','toxic','toxicspikes','transform','trick','whirlwind','willowisp','wish','yawn'];BattleMoveSearch.GOOD_WEAK_MOVES=['accelerock','acrobatics','aquajet','avalanche','biddybud','bluebites','bombsaway','bonemerang','bonetwirl','bouncybubble','bulletpunch','buzzybuzz','circlethrow','clearsmog','cosmicspin','crushgrip','cumuloroll','diffusionwave','doubleironbash','dragondarts','dragonroll','dragontail','endeavor','facade','firefang','flipturn','flyingpress','freezedry','frustration','geargrind','grassknot','gulchkick','gyroball','hex','icedrill','icefang','iceshard','iciclespear','infernalgaze','knockoff','lowkick','lunaticeyes','machpunch','mysticalfire','naughtyornice','nightshade','nuzzle','onetwopunch','payday','pikapapow','propulsionshot','psychocut','pursuit','quickattack','rapidspin','return','rockblast','savagehorn','scorchingsands','seismictoss','shadowclaw','shadowsneak','sizzlyslide','snugglebug','spiderweb','sporeslash','storedpower','stormthrow','suckerpunch','superfang','surgingstrikes','sweepup','tailslap','toxicthread','tripleaxel','triplekick','tropkick','twineedle','uturn','vacuumwave','vaporize','veeveevolley','voltswitch','watershuriken','weatherball'];BattleMoveSearch.BAD_STRONG_MOVES=['belch','burnup','crushclaw','doomdesire','dragonrush','dreameater','firepledge','futuresight','grasspledge','hyperbeam','hyperfang','hyperspacehole','jawlock','lastresort','mistyexplosion','muddywater','nightdaze','pollenpuff','selfdestruct','shelltrap','skyuppercut','slam','strength','synchronoise','takedown','thrash','uproar','waterpledge'];BattleMoveSearch.GOOD_DOUBLES_MOVES=['allyswitch','bulldoze','coaching','electroweb','faketears','fling','followme','healpulse','helpinghand','junglehealing','lifedew','muddywater','pollenpuff','psychup','ragepowder','safeguard','skillswap','snipeshot','wideguard'];var
